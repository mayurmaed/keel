...
export async function authCreate(
  ...
  if (!opts.db) throw new Error("auth needs a database — pass --db <name> (create one with `bareboat db create <name>`)");
  ...
  if (!db) throw new Error(`database "${opts.db}" not found — create it with \`bareboat db create ${opts.db}\``);
  ...
  const adminUrl = db.isolation === "shared"
    ? `postgres://bareboatadmin:${(await clients.ssm.send(new GetParameterCommand({ Name: "/bareboat/db-shared/master", WithDecryption: true }))).Parameter!.Value}@${db.host}:5432/${db.dbName}?sslmode=require`
    : (await clients.ssm.send(new GetParameterCommand({ Name: `/bareboat/db/${opts.db}/url`, WithDecryption: true }))).Parameter!.Value as string;
  const gotrueRole = `bareboatauth_${name.replace(/-/g, "_")}`;
  const gotruePw = randomBytes(24).toString("hex");
  await ensureAuthRole(io.pg ?? realPg, adminUrl, gotrueRole, gotruePw);

  const gotrueDbUrlParam = `/bareboat/auth/${name}/db-url`;
  ...
  const jwtSecretParam = `/bareboat/auth/${name}/jwt-secret`;
  ...
  await putAuth(reg, {
    ...
    host: new URL(url).host, port: albPort, stack: `bareboat-auth-${name}`, taskSgId, url,
    createdAt: new Date().toISOString(),
  });
  recordResource({
    kind: "auth", name, project: opts.project ?? name, region: gcfg.region,
    stack: `bareboat-auth-${name}`, url, createdAt: new Date().toISOString(),
  }, io.projectsPath);
  console.log(`auth "${name}" ready: ${url}`);
  console.log(`JWT secret in SSM: ${jwtSecretParam}`);
  console.log(`link an app by adding "auth": "${name}" to its bareboat.json`);
}

export async function authList(io: AwsIo = {}): Promise<void> {
  const { reg } = awsDeps(io);
  const list = await listAuths(reg);
  if (!list.length) { console.log("no auth services — create one with `bareboat auth create <name> --db <db>`"); return; }
...
export async function authUrl(name: string, io: AwsIo = {}): Promise<void> {
  ...
  if (!a) throw new Error(`auth "${name}" not found — create it with \`bareboat auth create ${name} --db <db>\``);
  console.log(a.url);
}
...
export async function authDestroy(name: string, io: AwsIo = {}): Promise<void> {
  ...
  const stackName = `bareboat-auth-${name}`;
  ...
  await clients.ssm.send(new DeleteParameterCommand({ Name: `/bareboat/auth/${name}/jwt-secret` })).catch(() => {});
  await clients.ssm.send(new DeleteParameterCommand({ Name: `/bareboat/auth/${name}/db-url` })).catch(() => {});
