...
account**. Bareboat is a self-hosted Render/Supabase alternative: you own the
infrastructure and pay only for the AWS resources you choose to run.

Bareboat has three core pieces:
...
![bareboat new, deploy, status, curl, logs, destroy — the full local-Docker flow in one take](demo/bareboat-demo.gif)
...
> **Note:** this recording predates the rename, so it shows the commands typed as
> `keel` rather than `bareboat`. The flow is otherwise unchanged. Re-record with
> `demo/run.sh`.

## End-to-end AWS walkthrough

This walkthrough creates a Bareboat control plane, a database, an auth service, and
an app linked to both. It makes AWS resources; do it in an account and region
you control.

### Prerequisites

- Node.js 20 or newer, and the Bareboat CLI installed:

  ```bash
  npm install -g @mayurmaed/bareboat   # installs the `bareboat` command
  ```

  (Scoped because the unscoped `bareboat` name on npm belongs to an unrelated project.)
...
bareboat setup --profile bareboat --region ap-south-1 \
  --github-token <fine-grained-token>
```

`bareboat setup` creates the control plane in your AWS account and remembers the
profile and its resource names in `~/.bareboat/config.json`.
...
```bash
bareboat db create my_app_db
bareboat auth create my-app-auth --db my_app_db
```

Use `bareboat db list` and `bareboat auth list` to inspect the resources, or
`bareboat auth url my-app-auth` to print the GoTrue base URL.
...
`bareboat new` writes `bareboat.json`, registers the app, and prints the GitHub webhook
...
For a local Docker-only app, run `bareboat new --target local` and `bareboat deploy`;
...
bareboat db list
bareboat auth list
bareboat logs --follow
bareboat env set KEY=value
bareboat destroy
```

`bareboat destroy` removes the current app's AWS resources (or its local container).
Remove data and auth resources deliberately with `bareboat db destroy <name>` and
`bareboat auth destroy <name>`.

## Status

- [x] Deploy pipeline: `bareboat setup`, GitHub webhook, CodeBuild, ECR, ECS/Fargate,
