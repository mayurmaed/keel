...
function makeEnv() {
  ...
  const clients = { cfn: { send: cfnSend }, ssm: { send: ssmSend }, ddb: { send: ddbSend } } as any;
  return { calls, ssmParams, dbRecords, authRecords, clients };
}
...
describe("authCreate", () => {
  it("creates its stack, JWT secret, registry record, and URL", async () => {
    const { calls, ssmParams, dbRecords, authRecords, clients } = makeEnv();
    dbRecords.set("appdb", { name: "appdb", dbSgId: "sg-db" });

    const { logs } = await withoutLogs(() => authCreate("appauth", { db: "appdb" }, { gcfg, clients }));
  ...
  it("allocates auth listener ports at 8100 or higher", async () => {
    const { calls, dbRecords, authRecords, clients } = makeEnv();
    dbRecords.set("appdb", { name: "appdb", dbSgId: "sg-db" });
    authRecords.set("existing", { name: "existing", port: 8105 });
    await withoutLogs(() => authCreate("appauth", { db: "appdb" }, { gcfg, clients }));
